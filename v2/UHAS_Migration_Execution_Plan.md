...
11. **Pre-go-live gap audit + follow-ups** — a 65-agent audit workflow swept the codebase (orphaned routes, unwired backend constants/enums, endpoints with no frontend consumer, dead code, hardcoded values, comment rot, speculative abstractions) ahead of go-live, plus a separate backlog of items the user flagged directly (onboarding checklist, dashboard data depth, academic-year/term management, search nav, promotions, staff/parent onboarding email, parent fee receipts). Sequenced by production-blocking risk and blast radius, not raw severity count — cheap/no-design-decision fixes first, then anything that blocks actually onboarding real users, then foundational data model work, then everything downstream of it.
    - ✅ **Tier 1 — cheap audit fixes** — done. Wired 2 fully-built-but-unreachable pages (`/admin/subjects`, the per-student report-card page); deleted confirmed-dead code re-verified with a fresh grep immediately before removal (the superseded `components/dashboard/` directory, 3 unused client hooks, 3 unused backend repository methods, the never-reachable `'button'` variant on both `DocumentDownloadLink` components); deduped `StaffTable`/`StaffDetail`'s identical role-label maps into `features/staff/role-styles.ts`.
    - ✅ **Tier 2 — Account emails: real provider + branded invite/reset/change** — done. Originally scoped as "staff onboarding invite email," expanded during scoping (at the user's direction) to cover the identical gap for parent accounts and the other two Supabase-handled account emails. `provision_login` (shared by staff + guardian logins), password recovery, and email-change confirmation all previously either used Supabase's own generic emailer or bypassed the backend entirely (`resetPasswordForEmail`/`updateUser({email})` called directly client-side). All three now mint their link via Supabase's admin `generate_link` API and send it through the branded Jinja2 system built in item 8, instead of Supabase's own mailer — verified against the local Supabase CLI stack that `generate_link` genuinely supports secure dual-confirmation email-change and that an unknown email raises a distinguishable error, load-bearing for the new enumeration-safe `POST /auth/reset-password` (public, IP-rate-limited, 5-minute per-account cooldown via new `users.last_password_reset_sent_at`). New `POST /staff/{id}/login` mirrors the existing guardian equivalent and replaces `StaffRegistrationForm.tsx`'s dead placeholder invite link with a real action. Production email provider is now Resend; local dev is Mailpit (`docker compose up mailpit`) — `SmtpEmailProvider` gained optional auth so the same class serves both. See `docs/superpowers/specs/2026-07-13-account-emails-provider-design.md`.
    - Remaining backlog (not yet sequenced into a PR): dashboard data enrichment/validation, academic-year/term management deep-dive, search navigation revisit, promotions revisit, first-time-setup onboarding checklist, parent-facing fee receipts, and the medium/low-severity audit findings not covered by tier 1 (unwired enum values, endpoints with no frontend consumer, response fields never read by the frontend) — to be folded into whichever future PR naturally touches that area.
