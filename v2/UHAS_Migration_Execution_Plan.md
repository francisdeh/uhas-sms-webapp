...
- **Profile page completion** — Save Changes ✅ done, Notification preferences ✅ done, self-service deactivation ✅ done; 2FA and Active Sessions are still UI-only. Full punch list in [`docs/implementation-spec.md`](../docs/implementation-spec.md#next-up--profile-page-completion) (stale on specifics — re-audit before picking each one up, per the pattern established on every Phase 3.5 item so far).
