...
- Guardians + siblings + staff-as-guardian — split into 3 dependency-ordered slices (the guardian↔student link surface was entirely missing; links existed only via the seed script):
  ...
  - **Slice 2 — guardian logins + phone-OTP provisioning** (pending): logins are already per-guardian, but the production provisioning path only sets email, so phone-OTP works for no non-seeded guardian — extend the Supabase admin surface + invite UX; consider a uniqueness guard on `users.linked_id`.
  - **Slice 3 — staff-as-guardian + staff-children marker/filter** (pending): materialize a `guardians` row for a staff member (with a `staff_id` back-reference), which then yields the staff-children filter on the admin students list. *(Moved here from item 6 — it needs the proper student↔staff link this item designs, not a bare FK.)*
