...
- **Profile page completion** — Save Changes ✅ done, Notification preferences ✅ done, self-service deactivation ✅ done, Active Sessions ✅ done (reframed to "sign out other devices" — Supabase exposes no per-session list); 2FA is the last UI-only piece. Full punch list in [`docs/implementation-spec.md`](../docs/implementation-spec.md#next-up--profile-page-completion) (stale on specifics — re-audit before picking each one up, per the pattern established on every Phase 3.5 item so far).
...
**Done when:** report cards render as real PDFs ✅, rate limiting exists on the routes that need it ✅, Admin Settings and Profile pages are fully wired (no UI-only stubs) — still open.
