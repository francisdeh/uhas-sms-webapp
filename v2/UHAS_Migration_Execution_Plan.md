...
8. **Close the email/SMS gaps, app-wide** — expanded scope after a full audit found the gap was much bigger than "appointments": a table of every notification-worthy event (lesson plans, schemes, announcements, attendance, results, leave requests, promotions, assignments, appointments, fees) showed most have in-app only, no email or SMS; `announcements`' "email a copy" toggle turned out to not exist in code at all (not even wired, let alone broken); `appointments.cancel` had zero notification of any kind (not even in-app); `results_published`'s email opt-out preference exists on the backend but no frontend UI ever sets it; and a generic, already-built `sms/jobs/sms_fanout.py` fan-out job sat unused with nothing emitting to it. Scoped into 4 sequential PRs:
   ...
   - Announcements' real email delivery, promotions, assignments, and schemes email stayed backlog items beyond this initiative — now picked back up as a second wave:
   - ✅ **PR 6 — Brevo email + Hubtel SMS provider wiring** — done. Full write-up in [`docs/CHANGELOG.md`](../docs/CHANGELOG.md) under "Brevo email + Hubtel SMS provider wiring". The school has since settled on real providers — Brevo for email, Hubtel for SMS (superseding PR 1's Arkesel-primary choice above) — so this PR replaces `ResendEmailProvider` (never configured with real credentials) with a new `BrevoEmailProvider`, and swaps SMS precedence so Hubtel (already fully implemented since PR 1, just previously de-prioritized) takes over as primary. Foundational groundwork before the 4 domain-specific email/SMS PRs below, since each of those will actually send through whichever provider is active.
   - ✅ **PR 7 — Supabase Send SMS hook (phone-OTP → Hubtel)** — done. Full write-up in [`docs/CHANGELOG.md`](../docs/CHANGELOG.md) under "Supabase Send SMS hook — relay phone-OTP through Hubtel". Kept deliberately separate from PR 6 — a new public webhook endpoint with HMAC signature verification is a different risk profile than a provider-precedence swap. Parent phone-OTP sign-in never actually delivered a real SMS in production (the `[auth.sms.twilio]` block was always a placeholder to satisfy Supabase's "at least one provider" requirement); a new `POST /auth/send-sms-hook` now intercepts every real OTP send and relays it through Hubtel. Fails closed on missing config — the one deliberate exception to this codebase's "missing config isn't an error" convention, since silence here would mean unsigned requests get accepted.
   - ✅ **PR 8 — Assignment notifications: email + SMS** — done, first of the 4 domain PRs. Full write-up in [`docs/CHANGELOG.md`](../docs/CHANGELOG.md) under "Assignment notifications: email + SMS". `ParentsOfClassAudience` (the generic audience resolver the in-app-only notification used before this PR) only returns bare `users.id`s, insufficient for email/SMS which need the guardian's phone/email — so, mirroring how PR 4 solved the identical problem for attendance, a new dedicated `AssignmentsRepository.list_primary_guardians_for_class` query replaces it entirely in `AssignmentsService.publish()`. Same two-tier gate shape as every prior PR in this initiative (`notification_defaults.on_assignment_created` + per-user `{email,sms}_on_assignment_created`), defaulting **true** unlike attendance's deliberate opt-out — an assignment-posted event doesn't share attendance's daily-volume/sensitivity profile. New `"assignment"` SMS category added to the closed set.
   - ✅ **PR 9 — Scheme notifications: email + SMS** — done, second of the 4 domain PRs. Full write-up in [`docs/CHANGELOG.md`](../docs/CHANGELOG.md) under "Scheme notifications: email + SMS". Structurally different from PR 8 — Schemes already had 3 in-app-only notification kinds across 2 directions (Unit-Head-facing "activity": submit + teacher-comment; teacher-facing "decided": acknowledge + reviewer-comment), the same shape `LeaveRequestsService` (PR 3) already solved, so this PR mirrors `_notify_leave_channels`/`_notify_leave_approvers`/`_notify_leave_requester` rather than PR 8's guardian-fan-out shape. Two scope decisions confirmed with the user before implementing: the Unit-Head-only audience stays as-is (no Deputy Head/Admin fallback — a separate decision from "add channels"), and preferences are two pairs by direction (`on_scheme_activity`/`on_scheme_decided`), not four by event. New `"scheme"` SMS category; 3 new email templates/jobs (the comment one shared by both directions). Fixed a real copy bug found during manual verification: the submit button read "Submit to Head of School," but the actual recipient is the Unit Head only — changed to "Submit for review."
   - ✅ **PR 10 — Promotion notifications: email + SMS** — done, third of the 4 domain PRs. Full write-up in [`docs/CHANGELOG.md`](../docs/CHANGELOG.md) under "Promotion notifications: email + SMS". Biggest of the three so far — 5 in-app-only notification kinds across 3 recipient shapes, not 2: `PROMOTION_SEASON_OPENED` (all-teachers broadcast on season open), `PROMOTION_SUBMITTED` (reviewer-facing: Deputy Head + Admin, same dual-eligibility shape as PR 3/PR 9), and `PROMOTION_SENT_BACK`/`PROMOTION_APPROVED`/`PROMOTION_REMINDER` (teacher-facing, about their own submission). Two scope decisions confirmed with the user before implementing: the season broadcast gets email + SMS too despite being a mass send (infrequent — ~1-2×/year at term boundaries — so the cost/spam concern doesn't outweigh consistency with every other event here), and preferences are 3 pairs by direction (`on_promotion_season`/`on_promotion_activity`/`on_promotion_decided`, folding sent-back + approved + reminder into one "decided" bucket), not 5 by event. New `"promotion"` SMS category; 5 new email templates/jobs (one per event, since unlike PR 9's shared "commented" template, each of these 5 has genuinely distinct content). Renamed the shared Admin/DeputyHead preference-row array from `LEAVE_APPROVER_PREFERENCE_ROWS` to `ADMIN_DEPUTY_PREFERENCE_ROWS` since it now also carries promotion-activity rows. Also switched local dev from Webpack to Turbopack (`next dev --turbopack`) in the same PR — an unrelated but noticeably-faster fix found the same session; `--webpack` had been the unexamined scaffold default since the project's first commit.
   - **Next up**: Announcements (needs a genuine product decision on how "email a copy" interacts with the existing `is_critical` flag, already scoped: recipient preference with a critical-always-emails override; also now scoped to add a staff-only audience variant to the core audience model itself, not just the notification channel, per a follow-up user decision).
...
11. **Pre-go-live gap audit + follow-ups** — a 65-agent audit workflow swept the codebase (orphaned routes, unwired backend constants/enums, endpoints with no frontend consumer, dead code, hardcoded values, comment rot, speculative abstractions) ahead of go-live, plus a separate backlog of items the user flagged directly (onboarding checklist, dashboard data depth, academic-year/term management, search nav, promotions, staff/parent onboarding email, parent fee receipts). Sequenced by production-blocking risk and blast radius, not raw severity count — cheap/no-design-decision fixes first, then anything that blocks actually onboarding real users, then foundational data model work, then everything downstream of it.
    ...
    - ✅ **Parent-facing fee receipts** — done. Full write-up in [`docs/CHANGELOG.md`](../docs/CHANGELOG.md) under "Parent-facing fee receipts"; see also `docs/superpowers/specs/2026-07-14-fee-receipts-design.md`. Reversed a prior deliberate exclusion so parents can download the Accountant's already-uploaded proof-of-payment file(s), rather than building a parallel system-generated PDF pipeline.
    - ✅ **Dashboard data enrichment/validation** — done. Full write-up in [`docs/CHANGELOG.md`](../docs/CHANGELOG.md) under "Dashboard data enrichment/validation"; see also `docs/superpowers/specs/2026-07-14-dashboard-data-enrichment-design.md`. Fixed a real, previously-undetected bug (Present/Late status casing) that silently zeroed out attendance stats across three dashboards; enriched all four role dashboards (Admin, DeputyHead, Teacher, Parent) with real data the backend already computed but never surfaced, and eliminated the Teacher dashboard's N+1 class lookup with a new `classTeacherId` filter on `GET /classes`.
    - ✅ **Teacher classes page N+1 fix** — done. Full write-up in [`docs/CHANGELOG.md`](../docs/CHANGELOG.md) under "Teacher classes page N+1 fix". Repointed `/teacher/classes` to the `classTeacherId` filter, closing the follow-up the dashboard-enrichment PR deliberately left open.
    - ✅ **Search navigation revisit** — done. Full write-up in [`docs/CHANGELOG.md`](../docs/CHANGELOG.md) under "Search navigation revisit"; see also `docs/superpowers/specs/2026-07-14-search-navigation-revisit-design.md`. Removed the dead Announcements search branch; fixed broken student/staff search-result navigation; added two new backend-already-permitted-but-UI-absent read-only pages (Teacher's student detail, Deputy Head's staff detail) plus their list-page + sidebar-nav siblings (added mid-implementation once the detail pages surfaced dead breadcrumb links — `AutoBreadcrumb.tsx` mechanically links every URL segment with no check that it resolves to a real page); expanded search to fee items/lesson plans/schemes with role scoping and a new `?focus=<id>` deep-link mechanism into the existing expand-in-place review pages.
    - ✅ **Medium/low-severity audit findings — PR 1: cleanup + missing edit UIs** — done. Full write-up in [`docs/CHANGELOG.md`](../docs/CHANGELOG.md) under "Audit findings: cleanup + missing edit UIs"; see also `docs/superpowers/specs/2026-07-14-audit-cleanup-edit-uis-design.md`. Deleted 4 confirmed-dead backend routes; added 5 missing edit dialogs (Exam, Class, Fee Item, Subject, Guardian contact-info) for domains with a working `PATCH` endpoint but no UI; two "dead code" candidates (guardian PATCH, notification mark-read) turned out to be real unbuilt features/bugs and were fixed properly instead of deleted; found and fixed a genuine "Mark all as read" bug during verification (wrong event prop for this codebase's Base UI menu primitive); centralized a `TERMS` constant across 6 hardcoded-literal sites; found and fixed DeputyHead's student profile showing edit/guardian actions the backend has always reserved for Admin only.
    - ✅ **Medium/low-severity audit findings — PR 2: promotions + attendance revisit** — done. Full write-up in [`docs/CHANGELOG.md`](../docs/CHANGELOG.md) under "Audit findings: promotions + attendance revisit". Wired up attendance's Excused status (backend already fully supported it; frontend never exposed it, and one page actively downgraded existing Excused records to Absent on save — a real data-corruption bug, now fixed); surfaced promotions' already-computed class-teacher names on Admin/DeputyHead detail pages and the DH queue list; found and fixed a pre-existing crash in DeputyHead's promotion review page (a response-shape mismatch that had apparently never been exercised until this PR's verification created the first live submission in the local dev database). Zero backend changes.
    - ✅ **Promotions revisit** — done. Full write-up in [`docs/CHANGELOG.md`](../docs/CHANGELOG.md) under "Promotions revisit". Closed the last of Phase 6's original vague gap-audit items. Added submit/approve in-app notifications, a comment-history thread (fixing real history loss on repeat send-backs), a weekly unsubmitted-class reminder job, bulk-approve with savepoint-isolated partial-failure handling, and a fix for Admin's permanently-wrong exam-published warning. Two more real bugs found and fixed during manual verification: Admin's overview grid never displayed a computed class-teacher name, and a manually-set "Repeat" decision always crashed at approval time (fixed by auto-deriving the target class server-side).
