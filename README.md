...
| Profile page completion | 🚧 In progress | Save Changes (display name + phone), Notification preferences, self-service account deactivation, and Active Sessions ("sign out other devices" via `signOut scope:'others'` — Supabase exposes no real per-device list) are all real now; photo upload + password change already were. Only 2FA remains a UI-only stub. Full punch list in [docs/implementation-spec.md](docs/implementation-spec.md#next-up--profile-page-completion). |
