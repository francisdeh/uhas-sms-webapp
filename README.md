...
| Promotions revisit | ✅ Done | Closed the last vague Phase 6 gap-audit item. Added submit/approve in-app notifications (previously silent — a DH had to manually check the queue); replaced the single overwriting `reviewer_comment` column with an append-only comment thread (fixed real history loss on repeat send-backs); added a weekly unsubmitted-class reminder job; added bulk-approve with savepoint-isolated partial-failure handling; fixed Admin's exam-published warning, which had been permanently wrong since launch. Found and fixed two more real bugs during verification: Admin's overview grid computed but never displayed each class's teacher name, and manually setting a decision to "Repeat" always crashed at approval time with no target class — fixed by auto-deriving it server-side. |
| Brevo email + Hubtel SMS provider wiring | ✅ Done | Removed `ResendEmailProvider` (never configured with real credentials) in favor of a new `BrevoEmailProvider`, now the primary provider; swapped SMS precedence so the already-fully-built `HubtelSmsProvider` (the school's chosen provider) takes priority over Arkesel. Foundational for the email/SMS delivery gaps still open on Announcements/Assignments/Promotions/Schemes. Fixed 3 places where deployment docs had gone stale describing this exact subsystem as unbuilt. |
| Supabase Send SMS hook — relay phone-OTP through Hubtel | ✅ Done | Parent phone-OTP sign-in never actually delivered a real SMS — Supabase's config had a placeholder Twilio provider block flagged "not implemented" months ago, so only test accounts worked. New `POST /auth/send-sms-hook` registers as Supabase's custom Send SMS hook, HMAC-verifying every request (`standardwebhooks`, the open spec Supabase itself implements) before relaying through the app's own Hubtel integration. Fails closed on missing config, unlike every other provider setting in the codebase — a security-relevant exception. Testable locally via Docker-to-host networking, not just in production. |
| Assignment notifications: email + SMS | ✅ Done | First of four domain PRs (Assignments → Schemes → Promotions → Announcements) extending the in-app-only notification pattern with email + SMS. New dedicated repository query replaces the generic audience resolver (which only returns bare user IDs, insufficient for email/SMS); fans out one in-app + one email + one SMS per guardian, deduped across multiple children in the same class. Two-tier gate (school + per-user prefs) defaults on, unlike attendance's deliberate opt-out. New `"assignment"` SMS category; new Parent profile + Admin settings preference rows. |
| Scheme notifications: email + SMS | ✅ Done | Second of four domain PRs. Two-direction (Unit-Head-facing "activity" + teacher-facing "decided"), mirroring `LeaveRequestsService`'s shape rather than Assignments' single-direction guardian fan-out — Schemes already had 3 in-app notification kinds across submit/acknowledge/comment. New shared `_notify_scheme_channels` helper + two audience helpers wired into all 3 call sites; new `"scheme"` SMS category; 3 new email templates/jobs. Fixed misleading "Submit to Head of School" button/list copy (the real recipient is the Unit Head only) found during manual verification. |
| Promotion notifications: email + SMS | ✅ Done | Third of four domain PRs, biggest so far — 5 in-app kinds across 3 recipient shapes (all-teachers broadcast on season open, reviewer-facing on submit, teacher-facing on sent-back/approved/reminder). 3 preference pairs, including a dedicated one for the season broadcast (confirmed in-scope despite being a mass send — infrequent enough not to matter). New `"promotion"` SMS category; 5 new email templates/jobs, one per event. Also switched local dev from Webpack to Turbopack (`next dev --turbopack`) — an unrelated but noticeably-faster fix found the same session. |
| Announcement notifications: email + SMS | ✅ Done | Fourth and final domain PR, closing out the "close the email/SMS gaps, app-wide" initiative. The odd one out — single-event domain, so complexity is on the audience side: new `:staff` audience suffix (`all:staff`, `division:JHS:staff`) is a real visibility distinction, not just a channel filter, so staff-only posts are invisible to parents in-app too. New `AllStaffAudience` resolver and a reusable mixed staff+parent contact resolver (`division:X` is the first scope needing both at once). One shared preference pair (no direction split); `is_critical` bypasses per-user prefs but never the school-level toggle; SMS batches all recipients into one event per audience spec while email stays per-recipient. |
| 16-domain correctness audit — Tier 0 security gates | ✅ Done | First fix batch from a 16-agent parallel domain audit run ahead of the Supabase cutover. Closed the same missing-role/ownership-gate shape independently found in 5 domains: attendance write, exam scores read/write, class-subjects/teachers/enrollment-roster reads, and assignment create/update all previously accepted any authenticated role (a Parent or Accountant could write attendance or scores for any class). New shared `ClassesService.assert_can_access_class` + `ClassesRepository.staff_teaches_class` primitives, reused across all 5; assignments additionally gained a subject-specific ownership check plus cross-tenant `subject_id` re-validation on update. Remaining audit findings (Tiers 1–5: promotions/appointments rollover bugs, account-safety gaps, non-atomic mutations, missing audit-log entries, UX/data-quality polish) tracked for follow-up PRs. |
| 16-domain correctness audit — Tier 1 rollover fixes | ✅ Done | Second fix batch, closing the audit's operational-blocker cluster. Fixed a real bug where cross-division promotions (e.g. Primary 6 → JHS 1) could never resolve a target class at all — every candidate-class query was filtering by the *current* class's division instead of considering the whole school; deleted the now-dead, same-shaped `divisionHasNextYearClasses`/`division_has_next_year_classes` helpers on both sides. Added the missing completeness guard to `activate_next_year` (blocks rollover unless every current-year class has an approved promotion — closes the exact gap that dropped 92/112 students' enrollment in a live incident this session) plus a real progress indicator + confirmation dialog on the Year Rollover card. Gave Admin the same approve/send-back UI Deputy Head already had (backend always supported it; only the page was missing the button). Fixed the appointments teacher-picker to apply the same promoted-but-not-yet-activated-year fallback the children list already uses, so parents of recently-promoted students can book appointments again. |
| 16-domain correctness audit — Tier 2 account/session safety | ✅ Done | Third fix batch. Deactivating a staff member's login now actually revokes their Supabase session (previously `staff.is_active` was flipped but never propagated to the linked `users` row or Supabase ban, so a deactivated Teacher/DeputyHead/Admin could keep working indefinitely); confirmation copy across Staff pages now matches. Changing a staff member's role now syncs `users.role` and the Supabase `app_metadata.role` the JWT actually reads — previously "changing" a role never touched what actually gated access, so the two could silently disagree forever. Added a guard blocking an Admin from deactivating their own account or the school's last active Admin. Accountant is now a selectable role in both the Staff registration and Change Role dialogs (it existed everywhere else — model, styling, permissions — but three hardcoded dropdowns never listed it), and the "division required" check now correctly applies to every non-Admin role instead of just Deputy Head/Teacher. Corrected a misleading comment describing the 2FA step-up gate as "un-bypassable" — it's a Next.js navigation gate only; FastAPI doesn't check the JWT's `aal`/`amr` claims today, and doing so properly needs a persisted enrollment flag that doesn't exist yet (tracked as separate follow-up, not attempted here). |
| 16-domain correctness audit — Tier 3 atomic mutations | ✅ Done | Fourth fix batch, closing the audit's non-atomic-mutation cluster: three flows that orchestrated a multi-step domain change as sequential independent API calls from the frontend, so a failure partway through left a record in a broken half-state. Student class transfer is now one `POST /enrollments/transfer` call (withdraw old + create new enrollment in a single DB transaction) instead of withdraw-then-create; class-teacher reassignment is now one `PUT /classes/{id}/teachers/primary` call (remove old primary + assign new in one transaction) instead of remove-then-add. Fixed a real data-loss bug in attendance: editing a historical session (Admin's "any past date" capability) silently dropped any student who'd since transferred out or been deactivated, because the roster-validation check only ever considered *today's* active roster; it now also admits students already present in that session's existing records, so resubmitting an old session preserves everyone's attendance instead of only whoever is still enrolled today. |
| 16-domain correctness audit — Tier 4 audit-log gaps | ✅ Done | Fifth fix batch, closing the audit's audit-log-coverage cluster across 8 domains and 13 mutation methods. The Fees domain was entirely unaudited (fee item edits, learner-fee edits/waives/exclusions, payment recording) despite being the money-handling surface most likely to need a trail; also closed a "3 of 4 log, one doesn't" inconsistency on student-guardian link edits, gave promotions' `send back` decision the same audit coverage its sibling `approve` already had, and added coverage for lesson-plan review decisions, staff/student activation toggles, Unit Head grants, and enrollment transfers/status changes. 17 new audit actions added to the closed `AuditAction` enum on both the FastAPI and TypeScript sides. |
| 16-domain correctness audit — Tier 5 UX/data-quality polish | ✅ Done | Sixth and final fix batch, closing the audit's lowest-severity cluster. Backfilled `loading.tsx` on 4 routes Deputy Head/Teacher had missing that their Admin equivalents already had; gave the Student detail page a working Deactivate/Reactivate control it never had (previously only reachable from the list page); standardized confirmation-dialog copy across Staff/Student/Fee destructive actions to one consistent voice (name-in-title, consistent reversibility language); fixed 4 date displays that bypassed the shared `lib/dates.ts` helper (3 of which were locale-dependent); replaced generic "Something went wrong" toast fallbacks with action-specific messages across 8 mutations; added `overflow-x-auto` to a subjects table using a raw `<table>` element; standardized fee-amount input defaults to always show cents. Also documented the conventions this and the two prior tiers surfaced — atomic multi-step mutations, audit-log coverage checks, confirmation-dialog copy, loading-boundary parity across roles, and specific toast messages — as new/expanded rules in `docs/ENGINEERING-CONVENTIONS.md` so future work doesn't reintroduce the same gaps. |
| Pre-deploy verification pass — report cards, account flows, UX gaps | ✅ Done | Fixed a real BECE-methodology bug found during manual pre-deploy QA: the report-card aggregate summed *every* graded subject instead of the student's best 6, inflating a genuine student's aggregate from 43 to 69 — fixed on both the FastAPI and TypeScript sides, then the redundant TS-side fallback was removed entirely so the backend's value is the only source of truth. Fixed a real 404 bug where Admin couldn't open a report card for a student with no currently-active class (a withdrawn/transferred-out student's class now resolves via fallback instead of erroring, and the genuinely-never-enrolled case now shows an actionable in-app notice instead of Next.js's generic "page not found"). Fixed every modal in the app being unable to scroll past the viewport's height (a shared `Dialog`/`AlertDialog` primitive bug affecting all ~41 modals, not just the ones someone happened to notice). Rebuilt the "New account" modal: the Staff/Guardian picker no longer lists people who already have a login, picking one now auto-fills their name/email/phone from their own record, and Role/person selection moved to the top of the form so it drives the auto-fill instead of overwriting fields already typed below it. Added the missing "Create Login" button to the Staff detail page — the one-click `POST /staff/{id}/login` endpoint already existed server-side but had zero UI wired to it, so provisioning a staff login required going through the generic Users page instead. Fixed a real data-integrity bug: a duplicate `uhas_id` (which has a genuine DB-level unique constraint) fell through to the slug-collision retry path and surfaced as a misleading "could not generate a unique slug" error instead of a clear "this UHAS ID is already in use" one — worse on the update path, where it wasn't caught at all and would have been a raw 500. Corrected inaccurate copy claiming a staff member's phone becomes their login credential (staff sign in with email + password only; phone-OTP is parent-only — the number is actually used for SMS notifications). Added missing icons to several detail-page action buttons for visual consistency with their siblings. |
| Railway dev-deploy prep: SMS allowlist + seed-data safety | ✅ Done | Prep work for standing up a Railway dev environment (Supabase self-hosted template) ahead of the production cutover. New `AllowlistSmsProvider` wraps whichever real SMS provider is configured — outside production, only phone numbers on a new `sms_dev_allowlist` setting ever trigger a real (billed) Hubtel/Arkesel send; every other number is logged and handed a synthetic success, no network call, no cost. Deliberately different from email's existing `email_dev_redirect` (which resends *everything* to one address) — SMS is billed per-message, so dev/demo traffic should be suppressed, not rerouted. Also fixed the bulk-generated "extra staff" seed rows, which used real `@uhas.edu.gh` addresses instead of the `.example.com` domain the equivalent guardian-seeding loop already correctly used — standardized both to the safe, IANA-reserved domain. The 9 documented demo login accounts (`admin@uhas.edu.gh` etc.) are untouched — those are real, intentional credentials the team uses to sign in, not bulk demo noise. |
