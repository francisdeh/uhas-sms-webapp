...
- Photo upload, password change, Profile-tab Save Changes (display name + phone, via `PATCH /me`), Notification preferences, and self-service account **deactivation** are **real**
...
- **Self-service deactivation** (`POST /me/deactivate`): a non-Admin can deactivate their own account from the Profile "Danger Zone" — reuses the exact admin-deactivation mechanism (`UsersService.set_active`: flips `users.is_active` + issues the Supabase `ban_duration` ban), then the web client signs them out and redirects to `/login?deactivated=1`. Admins are blocked (403) to avoid a sole admin orphaning the school. Both the self path (`ACCOUNT_SELF_DEACTIVATED`) and the admin activate/deactivate paths (`USER_DEACTIVATED`/`USER_REACTIVATED`) now write audit rows — previously deactivation was audited nowhere. Reactivation stays admin-only (a deactivated user can't log in). **Enforcement note:** an inactive account is blocked *only* by the Supabase ban — nothing in `proxy.ts`/`security.py`/`deps.py` checks `users.is_active`, so the DB flag is currently informational. Design: `docs/superpowers/specs/2026-07-06-self-service-deactivation-design.md`.
- **Active Sessions**: the Security tab's old `MOCK_SESSIONS` device list (MacBook/iPhone/Windows with dead "Revoke" buttons) was fictional — the installed `@supabase/auth-js` (2.108.x) exposes **no** per-session enumeration (`GoTrueAdminApi` has no `listUserSessions`, and the `Session` type carries no device/UA/IP metadata). Replaced with a real **"Sign out other devices"** action — `supabase.auth.signOut({ scope: 'others' })`, which revokes every other session's refresh token while keeping the current one (verified: other session's refresh → 400, current → 200). No device labels, no per-row revoke; a genuine device list would require reading GoTrue's private `auth.sessions` table directly, deliberately rejected. Design: `docs/superpowers/specs/2026-07-06-active-sessions-design.md`.
- **2FA / TOTP** (real, opt-in, backed by Supabase Auth MFA — completes Phase 3.5): enrol from Profile → Security (`TwoFactorCard.tsx`: `mfa.enroll` → QR built from the returned otpauth URI via the `qrcode` lib → `mfa.challenge`+`verify` to activate; disable via `mfa.unenroll`). Login-time challenge is a new `"mfa"` stage in `LoginForm` (after password/OTP, `getAuthenticatorAssuranceLevel` → challenge when `currentLevel:'aal1'`/`nextLevel:'aal2'`). Un-bypassable via a **proxy gate** (`proxy.ts` + `middleware.ts` compute `needsMfa` and redirect enrolled `aal1` sessions to a standalone `/verify-2fa` page — catches the abandoned-challenge-then-direct-navigation path). **No backup codes** (Supabase doesn't support them); lockout recovery is an admin **`POST /users/{id}/reset-mfa`** (Python `admin.mfa.list_factors`+`delete_factor`, audit-logged `USER_MFA_RESET`, surfaced as a "Reset 2FA" action in the admin users table). Config: `supabase/config.toml` `[auth.mfa.totp]` enabled locally — **production must enable MFA in the Supabase dashboard** (see DEPLOY.md §2). Known limitation (documented, not built): the FastAPI API doesn't reject an `aal1` token from an enrolled user — the proxy gate enforces the web layer; per-request API AAL enforcement is a future-hardening item. Design: `docs/superpowers/specs/2026-07-06-2fa-totp-design.md`.
...
| Profile pages | ✅ done — Save Changes, Notification prefs, self-deactivation, Active Sessions, and 2FA/TOTP all real | — | — |
...
**Recommended priority order** — **Phase 3.5 is complete** (real report-card PDFs ✅, rate-limiting audit ✅, Admin Settings page ✅, and full Profile page completion incl. 2FA ✅). Next is **Phase 4** — the FRD requirement gaps (Common Core subjects, Scheme-of-Learning template, guardian limits, report-card fields).
...
- **Phase 3.5 — Platform completion & admin polish ✅ complete**: real report-card PDF rendering ✅, rate-limiting audit ✅, Admin Settings page ✅, Profile page completion ✅ (Save Changes, Notifications, self-deactivation, Active Sessions, 2FA/TOTP).
...
> "I'm working on UHAS SMS — a demo-phase school management system for a basic school in Ghana (KG → JHS 3), designed for ~350 students / ~50 staff at full scale. Five roles: Admin, Deputy Head (4 divisions), Teacher (with Unit Head flag), Parent, Accountant. Stack: Next.js 16 frontend (zero direct DB access) + FastAPI/SQLAlchemy/Alembic backend + Supabase (Postgres/Auth/Storage) + Inngest for background jobs. Went through a full backend migration off Drizzle/Firebase/Neon — that's Phases 0–3 done, tracked in `v2/UHAS_Migration_Execution_Plan.md`, which is the current roadmap (Phase 3.5 → 4 → 5 → 6 → 7). 540+ pytest tests on the backend, Vitest unit tests on the frontend; CI gates Railway deploys but Playwright E2E is currently disabled. Most features work end-to-end against real seed data; Phase 3.5 is complete, so fee management (Phase 5) and the Phase 4 requirement gaps are the biggest near-term work. Conventions live in `docs/ENGINEERING-CONVENTIONS.md`. Hand me [task]."
