...
- **2FA / TOTP** is the last remaining **UI-only** Profile sub-feature (the Security tab's authenticator/backup-codes flow is still mock — "2FA setup coming soon"). Supabase Auth has native MFA (`auth.mfa.enroll/challenge/verify`), so it's buildable, but it's the heaviest remaining item (enrollment UI + login-time enforcement + AAL handling). See the shipped specs in `docs/superpowers/specs/` as templates.
...
| Profile pages | Save Changes + Notification prefs + self-deactivation + Active Sessions ✅ done — only 2FA still UI-only | — | ~5 h |
...
**Recommended priority order** — Phase 3.5 is nearly done (real report-card PDFs ✅, rate-limiting audit ✅, Admin Settings page ✅, self-service deactivation ✅, Active Sessions ✅; only 2FA remains open) — see the migration plan for sequencing, then **Phase 4** for the FRD requirement gaps.
...
- **Phase 3.5 — Platform completion & admin polish**: real report-card PDF rendering ✅, rate-limiting audit ✅, Admin Settings page ✅, Profile page completion (Save Changes ✅, Notifications ✅, self-deactivation ✅, Active Sessions ✅; only 2FA still open).
