...
describe("createStudentAction", () => {
  it("generates next sequence ID and inserts student + active enrollment in a tx", async () => {
    const result = await createStudentAction({
      ...
      dob: "2015-05-15",
      gender: "Male",
      classId: det("class-p3"),
    });
    expect(result.success).toBe(true);
    if (!result.success) return;

    // The id is now a uuid; the human-readable identifier moved to .slug.
    ...
    expect(studentRow?.slug).toMatch(/^UHAS-\d{4}-\d{4}$/);
    ...
    expect(enr?.classId).toBe(det("class-p3"));
  });
  ...
  it("rejects unknown classId", async () => {
    const result = await createStudentAction({
      ...
      dob: "2015-05-15",
      gender: "Male",
      classId: det("class-does-not-exist"),
    });
...
describe("updateStudentAction", () => {
  it("updates fields and writes a STUDENT_EDIT audit log", async () => {
    ...
    const result = await updateStudentAction(det("UHAS-2026-0001"), {
      phone: "0244999000",
    ...
    const after = await db.query.students.findFirst({
      where: eq(students.id, det("UHAS-2026-0001")),
    });
    ...
    expect(last.targetId).toBe(det("UHAS-2026-0001"));
  });

  it("returns error when student not found", async () => {
    const result = await updateStudentAction(det("UHAS-0000-9999"), { phone: "x" });
    expect(result.success).toBe(false);
  });
});
...
describe("deactivate / reactivate", () => {
  it("flips isActive", async () => {
    await deactivateStudentAction(det("UHAS-2026-0002"));
    let row = await db.query.students.findFirst({
      where: eq(students.id, det("UHAS-2026-0002")),
    });
    expect(row?.isActive).toBe(false);

    await reactivateStudentAction(det("UHAS-2026-0002"));
  ...
  it("returns error for unknown id", async () => {
    const result = await deactivateStudentAction(det("UHAS-0000-0001"));
    expect(result.success).toBe(false);
  });
});
...
describe("transferStudentAction", () => {
  it("closes old enrollment and opens a new one in a tx", async () => {
    ...
    const before = await db.query.enrollments.findMany({
      where: eq(enrollments.studentId, det("UHAS-2026-0003")),
    });
    const activeBefore = before.find((e) => e.status === "Active");
    expect(activeBefore?.classId).toBe(det("class-jhs2"));

    const result = await transferStudentAction(det("UHAS-2026-0003"), {
      classId: det("class-jhs1"),
    });
    expect(result.success).toBe(true);
    ...
    expect(activeAfter?.classId).toBe(det("class-jhs1"));
    expect(completedAfter.length).toBeGreaterThanOrEqual(1);
    expect(completedAfter.find((e) => e.classId === det("class-jhs2"))).toBeDefined();
  });

  it("rejects transfer to the same class", async () => {
    const result = await transferStudentAction(det("UHAS-2026-0004"), {
      classId: det("class-jhs2"), // already in this class
    });
    expect(result.success).toBe(false);
  });
});
