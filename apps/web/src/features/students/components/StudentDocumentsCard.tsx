...
import {
  useStudentDocumentMutations,
  useStudentDocuments,
} from "@/features/students/hooks/use-student-guardians";
import { DOCUMENT_LABELS, type DocumentLabel } from "@/features/students/types";

interface StudentDocumentsCardProps {
  studentId: string;
  /** Admin only, per the backend gate on POST/DELETE — Deputy and
   *  parents can view/download but not manage. */
  canManage: boolean;
}

export function StudentDocumentsCard({ studentId, canManage }: StudentDocumentsCardProps) {
  const { data, isLoading } = useStudentDocuments(studentId);
  const { add, remove } = useStudentDocumentMutations(studentId);
      ...
      <CardContent className="pt-5 space-y-3">
        ...
        {canManage && (
          <div className="space-y-2 pt-2 border-t border-border/40">
            ...
            <Select value={label} onValueChange={(v) => setLabel(v as DocumentLabel)}>
