"""Promotion-list-approved teacher email.

Triggered by `email/promotion-approved.requested`, emitted from
`promotions.service._notify_promotion_teacher` (called from
`PromotionsService.approve`), only after both gates pass (school's
...
async def _run(ctx: inngest.Context) -> dict[str, Any]:
    ...
    text = (
        f"Hi,\n\n"
        f"{class_name}'s promotion list was approved.\n\n"
        f"View the list:\n{link}\n\n"
        f"— {school_name}\n"
    )
    html = render_email_template(
        "promotion_approved.html",
    ...
    async def _send() -> dict[str, Any]:
        ...
        result = await provider.send(
            EmailMessage(
                to=recipient_email,
                subject=f"Promotion list approved: {class_name}",
                text=text,
                html=html,
            )
        )
...
promotion_approved_email_job = inngest_client.create_function(
    fn_id="promotion-approved-email",
    trigger=inngest.TriggerEvent(event="email/promotion-approved.requested"),
)(_run)
