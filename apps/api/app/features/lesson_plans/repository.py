...
from sqlalchemy import Subquery, and_, asc, desc, func, or_, select
...
class LessonPlansRepository:
    ...
    @staticmethod
    async def find_review_queue_for_deputy(
        session: AsyncSession,
        school_id: UUID | str,
        division: str,
        *,
        ...
        """Deputy-Head review queue: `unit_head_approved` OR `submitted`
        (Deputy can also review the first step, per current TS behaviour)."""
        return await LessonPlansRepository._list_review_queue(
            session,
            school_id,
            division=division,
            statuses=("unit_head_approved", "submitted"),
            page=page,
            size=size,
        )

    @staticmethod
    async def find_review_queue_for_unit_head(
        session: AsyncSession,
        school_id: UUID | str,
        division: str,
        *,
        ...
        """Unit-Head review queue: `submitted` in their division."""
        return await LessonPlansRepository._list_review_queue(
            session,
            school_id,
            division=division,
    ...
    async def _list_review_queue(
        ...
        *,
        ...
        where = [
            ...
            or_(*(LessonPlan.status == s for s in statuses)),
        ]
        where_clause = and_(*where)
        ...
        rows_stmt = (
            ...
            .outerjoin(reviewer, reviewer.id == latest.c.reviewer_id)
            .where(where_clause)
            .order_by(asc(LessonPlan.updated_at))  # oldest-first: FIFO queue
            .offset(offset)
            .limit(size)
        )
        rows = [
