"""Scheme-acknowledged teacher email.

Triggered by `email/scheme-acknowledged.requested`, emitted from
`schemes.service._notify_scheme_teacher` (called from
`SchemesService.acknowledge`), only after both gates pass (school's
`notification_defaults.on_scheme_decided` + the teacher's own
`user_preferences.email_on_scheme_decided`) — gating and recipient
...
async def _run(ctx: inngest.Context) -> dict[str, Any]:
    ...
    text = (
        f"Hi,\n\n"
        f"{scheme_title} for {class_name} was acknowledged.\n\n"
        f"{comment_block}"
        f"View the scheme:\n{link}\n\n"
        f"— {school_name}\n"
    )
    html = render_email_template(
        "scheme_acknowledged.html",
    ...
    async def _send() -> dict[str, Any]:
        ...
        result = await provider.send(
            EmailMessage(
                to=recipient_email,
                subject=f"Scheme acknowledged: {scheme_title}",
                text=text,
                html=html,
            )
        )
...
scheme_acknowledged_email_job = inngest_client.create_function(
    fn_id="scheme-acknowledged-email",
    trigger=inngest.TriggerEvent(event="email/scheme-acknowledged.requested"),
)(_run)
