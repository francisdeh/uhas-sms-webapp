...
Two orthogonal state machines live here:
  ...
  2. `AppointmentSlot` — which time block the parent requested:
       morning / afternoon / after_school

Both mirror the TS unions in
[apps/web/src/features/appointments/types.ts](../../../../web/src/features/appointments/types.ts).
Slot labels live on the frontend — the DB just stores the token so the
schedule can be reworded without a migration.
"""
...
AppointmentSlot = Literal["morning", "afternoon", "after_school"]
